...
export function init(server: Koa, container: ServiceContainer) {
  const router = new Router({ prefix: '/api/v1/members' })
  const controller = new MemberController(container.managers.member)
  ...
  router.post(
    ...
    middleware.authentication(container.lib.authenticator),
    middleware.authorization([Role.user, Role.admin]),
    middleware.validate({ request: { body: validators.createMember } }),
