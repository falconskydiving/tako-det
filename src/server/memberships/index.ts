...
export function init(server: Koa, container: ServiceContainer) {
  const router = new Router({ prefix: '/api/v1/memberships' })
  const controller = new MembershipController(container.managers.membership)
  ...
  router.post(
    ...
    middleware.authentication(container.lib.authenticator),
    middleware.authorization([Role.user, Role.admin]),
    middleware.validate({ request: { body: validators.createMembership } }),
  ...
  router.put(
    ...
    middleware.validate({
      ...
      request: {
        body: validators.updateMembership
      }
    }),
